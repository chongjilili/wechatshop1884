...
class CategoryController extends PublicController{
	...
	*/
	public function _initialize(){
		...
		// 获取所有分类，进行关系划分
		$list = $this->category->where('tid=0 AND bz_4<2')->order('sort desc,id asc')->field('id,tid,name,bz_2,bz_4,sort')->select();
		foreach ($list as $k1 => $v1) {
			$list[$k1]['list2'] = $this->category->where('tid='.intval($v1['id']))->field('id,tid,name,bz_2,sort')->select();
			foreach ($list[$k1]['list2'] as $k2 => $v2) {
				$list[$k1]['list2'][$k2]['list3'] = $this->category->where('tid='.intval($v2['id']))->field('id,tid,name,bz_2,sort')->select();
			}
		}
	...
	*/
	 ...
	 *
	 * */
	public function getChildrenCategoryById(){
		$id = I('id');
		$chilrencat = $this->category->where('tid='.$id)->field('id,tid,name,bz_2,sort')->select();

		echo json_encode($chilrencat);
